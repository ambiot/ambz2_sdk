...
#if LWIP_IPV6
#define IP6_CLIENT_BUF_SIZE     1440 // 1440 = 1500 - 20 - 40, IPv6 header length 40 bytes
#endif
#define KB                      1024
...
struct iperf_data_t {
	...
	uint16_t port;
#if LWIP_IPV6
	uint8_t  use_ip6_addr;
	uint8_t  server_ip[40];
#else
...
#if LWIP_IPV6
	struct sockaddr_in6 ser_addr6, client_addr6;
...
#if LWIP_IPV6
	if (iperf_data.use_ip6_addr) {
		iperf_data.buf_size = IP6_CLIENT_BUF_SIZE;
	}
#endif
...
#if LWIP_IPV6
	if (iperf_data.use_ip6_addr) {
		iperf_data.client_fd = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
	} else
#endif
	{
		iperf_data.client_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	}
	if (iperf_data.client_fd < 0) {
...
#if LWIP_IPV6
	if (iperf_data.use_ip6_addr) {
		memset(&ser_addr6, 0, addrlen6);
		ser_addr6.sin6_family = AF_INET6;
		ser_addr6.sin6_port = htons(iperf_data.port);
		inet_pton(AF_INET6, (char const *)iperf_data.server_ip, &(ser_addr6.sin6_addr));
	} else
#endif
	{
...
#if LWIP_IPV6
	// bind ipv6 link local address if the dest address is link local address
	if (iperf_data.use_ip6_addr) {
		inet6_addr_to_ip6addr(&dest_addr6, &(ser_addr6.sin6_addr));

		if (ip6_addr_islinklocal(&dest_addr6)) {
			memset(&client_addr6, 0, addrlen6);
			client_addr6.sin6_family = AF_INET6;
			client_addr6.sin6_port = 0;
			inet6_addr_from_ip6addr(&client_addr6.sin6_addr, (ip6_addr_t *)&xnetif[0].ip6_addr[0]);

			ret = bind(iperf_data.client_fd, (struct sockaddr *)&client_addr6, addrlen6);
			if (ret < 0) {
...
#if LWIP_IPV6
	if (iperf_data.use_ip6_addr) {
		ret = connect(iperf_data.client_fd, (struct sockaddr *)&ser_addr6, addrlen6);
	} else
#endif
	{
		ret = connect(iperf_data.client_fd, (struct sockaddr *)&ser_addr, addrlen);
	}
	if (ret < 0) {
...
int tcp_server_func(struct iperf_data_t iperf_data)
{
...
#if LWIP_IPV6
	if (iperf_data.use_ip6_addr) {
		iperf_data.server_fd = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
	} else
#endif
	{
		iperf_data.server_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	}
	if (iperf_data.server_fd < 0) {
	...
	if (iperf_data.use_ip6_addr) {
		...
		ser_addr6.sin6_family = AF_INET6;
		ser_addr6.sin6_port = htons(iperf_data.port);
		ser_addr6.sin6_addr = (struct in6_addr) IN6ADDR_ANY_INIT;
	} else
#endif
	{
		memset(&ser_addr, 0, addrlen);
...
#if LWIP_IPV6
	if (iperf_data.use_ip6_addr) {
		ret = bind(iperf_data.server_fd, (struct sockaddr *)&ser_addr6, addrlen6);
	} else
#endif
	{
		ret = bind(iperf_data.server_fd, (struct sockaddr *)&ser_addr, addrlen);
	}
	if (ret < 0) {
		tptest_res_log("\n\r[ERROR] %s: Bind socket failed\n\r", __func__);
		goto exit2;
...
#if LWIP_IPV6
				if (iperf_data.use_ip6_addr) {
					iperf_data.client_fd = accept(iperf_data.server_fd, (struct sockaddr *)&client_addr6, (u32_t *)&addrlen6);
				} else
#endif
				{
					iperf_data.client_fd = accept(iperf_data.server_fd, (struct sockaddr *)&client_addr, (u32_t *)&addrlen);
				}
				if (iperf_data.client_fd < 0) {
...
#if LWIP_IPV6
			if (iperf_data.use_ip6_addr) {
				strncpy((char *)tcp_client_data->server_ip, inet6_ntoa(client_addr6.sin6_addr), (strlen(inet6_ntoa(client_addr6.sin6_addr))));
			} else
#endif
			{
...
#if LWIP_IPV6
			tcp_client_data->use_ip6_addr = iperf_data.use_ip6_addr;
#endif
...
#if LWIP_IPV6
	if (iperf_data.use_ip6_addr) {
		iperf_data.client_fd = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
	} else
#endif
	{
		iperf_data.client_fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	}
	if (iperf_data.client_fd < 0) {
...
#if LWIP_IPV6
			if (iperf_data.use_ip6_addr) {
				ret = sendto(iperf_data.client_fd, udp_client_buffer, iperf_data.buf_size, 0, (struct sockaddr *)&ser_addr6, addrlen6);
			} else
#endif
			{
				ret = sendto(iperf_data.client_fd, udp_client_buffer, iperf_data.buf_size, 0, (struct sockaddr *)&ser_addr, addrlen);
			}
			if (ret < 0) {
...
#if LWIP_IPV6
		if (iperf_data.use_ip6_addr) {
			sendto(iperf_data.client_fd, udp_client_buffer, iperf_data.buf_size, 0, (struct sockaddr *)&ser_addr6, addrlen6);
		} else
#endif
		{
...
#if LWIP_IPV6
	if (iperf_data.use_ip6_addr) {
		iperf_data.server_fd = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
	} else
#endif
	{
		iperf_data.server_fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	}
	if (iperf_data.server_fd < 0) {
		tptest_res_log("\n\r[ERROR] %s: Create socket failed\n\r", __func__);
		goto exit2;
...
#if LWIP_IPV6
				if (iperf_data.use_ip6_addr) {
					recv_size = recvfrom(iperf_data.server_fd, udp_server_buffer, iperf_data.buf_size, 0, (struct sockaddr *) &client_addr6, (u32_t *)&addrlen6);
				} else
#endif
				{
...
#if LWIP_IPV6
			if (iperf_data.use_ip6_addr) {
				strncpy((char *)udp_client_data->server_ip, inet6_ntoa(client_addr6.sin6_addr), (strlen(inet6_ntoa(client_addr6.sin6_addr))));
			} else
#endif
			{
...
#if LWIP_IPV6
			udp_client_data->use_ip6_addr = iperf_data.use_ip6_addr;
#endif
...
#if LWIP_IPV6
				if (iperf_data.use_ip6_addr) {
					sendto(iperf_data.server_fd, udp_server_buffer, 0, 0, (struct sockaddr *)&client_addr6, addrlen6);
				} else
#endif
				{
					sendto(iperf_data.server_fd, udp_server_buffer, 0, 0, (struct sockaddr *)&client_addr, addrlen);
				}
...
#if LWIP_IPV6
			else if (strcmp(argv[argv_count - 1], "-V") == 0) {
				stream_data->use_ip6_addr = 1;
				argv_count += 1;
			}
#endif
			else {
