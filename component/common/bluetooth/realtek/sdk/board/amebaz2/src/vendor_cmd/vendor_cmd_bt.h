...
#define BT_VENDOR_CMD_ADV_POWER_SAVE_SUPPORT    1
...
#define HCI_LE_VENDOR_ADV_POWER_SAVE			0xFD92
#define HCI_EXT_SUB_ADV_EARLY		            1
#define HCI_EXT_SUB_ADV_END			            0
...
/**
  * @brief  Save the advertising tx power for beacon function.
  *
  *         NOTE: This function can be called on BT beacon only without WiFi connection.
  *
  ...
  * \code{.c}
     ...
     {
        T_GAP_CAUSE cause;
        cause = le_adv_set_power_save_enable(1);
     }
     void app_vendor_callback(uint8_t cb_type, void *p_cb_data)
     {
         T_GAP_VENDOR_CB_DATA cb_data;
         ...
         case GAP_MSG_VENDOR_EVT_INFO:
             ...
             switch(subcode)            
             {
             case HCI_VENDOR_PRE_ADV_EVENT_EARLY:
                 rtlk_bt_set_rf_power(1);
                 break;
             case HCI_VENDOR_PRE_ADV_EVENT_END:
                 rtlk_bt_set_rf_power(0);
                 break;
             }
             break;
       }
