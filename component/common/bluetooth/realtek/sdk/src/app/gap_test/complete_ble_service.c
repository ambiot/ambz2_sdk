...
#define GATT_UUID_CHAR_SIMPLE_V8_NOTIFY_INDICATE    0xB008
...
static uint8_t simple_v1_read_value = 0xf0;
uint8_t v7_char_value[SIMP_READ_V7_MAX_LEN] = {0};
uint16_t v7_char_value_len = SIMP_READ_V7_MAX_LEN;
...
const T_ATTRIB_APPL simple_ble_service_tbl[] =
{
    ...
    /* <<Characteristic>> demo for write */
    {
        ...
        {                                           /* type_value */
            LO_WORD(GATT_UUID_CHARACTERISTIC),
            HI_WORD(GATT_UUID_CHARACTERISTIC),
            (GATT_CHAR_PROP_WRITE | GATT_CHAR_PROP_WRITE_NO_RSP | GATT_CHAR_PROP_WRITE_AUTHEN_SIGNED) /* characteristic properties */
            /* characteristic UUID not needed here, is UUID of next attrib. */
        },
        1,                                          /* bValueLen */
        NULL,
    ...
    /* <<Characteristic>> demo for read Long*/
    {
        ATTRIB_FLAG_VALUE_INCL,                     /* flags */
        {                                           /* type_value */
            LO_WORD(GATT_UUID_CHARACTERISTIC),
...

    {
        ATTRIB_FLAG_VALUE_APPL,                     /* flags */
        {                                         /* type_value */
            LO_WORD(GATT_UUID_CHAR_SIMPLE_V7_READ_LONG),
            HI_WORD(GATT_UUID_CHAR_SIMPLE_V7_READ_LONG)
        },
        0,                                          /* bValueLen */
        NULL,
        GATT_PERM_READ                              /* permissions */
    ...
    /* <<Characteristic>> demo for indicate */
    {
        ...
        {                                           /* type_value */
            LO_WORD(GATT_UUID_CHARACTERISTIC),
            HI_WORD(GATT_UUID_CHARACTERISTIC),
            (GATT_CHAR_PROP_INDICATE | GATT_CHAR_PROP_NOTIFY) /* characteristic properties */
            /* characteristic UUID not needed here, is UUID of next attrib. */
        },
        1,                                          /* bValueLen */
        NULL,
...

    {
        ATTRIB_FLAG_VALUE_APPL,                     /* flags */
        {                                         /* type_value */
            LO_WORD(GATT_UUID_CHAR_SIMPLE_V8_NOTIFY_INDICATE),
            HI_WORD(GATT_UUID_CHAR_SIMPLE_V8_NOTIFY_INDICATE)
        },
        0,                                          /* bValueLen */
        NULL,
        GATT_PERM_NONE                              /* permissions */
...
bool simp_ble_service_set_parameter(T_SIMP_PARAM_TYPE param_type, uint16_t len, void *p_value)
{
    ...
    case SIMPLE_BLE_SERVICE_PARAM_V1_READ_CHAR_VAL:
        if (len == 1)
        {
            simple_v1_read_value = *(uint8_t *)p_value;
        }
        else
        {
            ret = false;
    ...
    case SIMPLE_BLE_SERVICE_PARAM_V7_LEN:
        if (len <= SIMP_READ_V7_MAX_LEN)
        {
            memcpy(v7_char_value, p_value, len);
            v7_char_value_len = len;
        }
        else
        {
            ret = false;
    ...
    case SIMPLE_BLE_SERVICE_CHAR_V1_READ_INDEX:
        {
            ...
            *pp_value = &simple_v1_read_value;
            *p_length = sizeof(simple_v1_read_value);
        }
        break;
    case SIMPLE_BLE_SERVICE_CHAR_V7_READ_LONG_INDEX:
        {
            TSIMP_CALLBACK_DATA callback_data;
            callback_data.msg_type = SERVICE_CALLBACK_TYPE_READ_CHAR_VALUE;
            callback_data.msg_data.read_value_index = SIMP_READ_V7;
            callback_data.conn_id = conn_id;
            if (pfn_simp_ble_service_cb)
            {
                pfn_simp_ble_service_cb(service_id, (void *)&callback_data);
            }
            *pp_value = v7_char_value;
            *p_length = v7_char_value_len;
        }
        break;
    }
                                            ...
                                            P_FUN_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    ...
    *p_write_ind_post_proc = simple_write_post_callback;
    ...
    else if (SIMPLE_BLE_SERVICE_CHAR_V6_WRITE_INDEX == attrib_index)
    {
        callback_data.msg_type = SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE;
        callback_data.conn_id  = conn_id;
        callback_data.msg_data.write.opcode = SIMP_WRITE_V6;
        callback_data.msg_data.write.write_type = write_type;
        callback_data.msg_data.write.len = length;
        callback_data.msg_data.write.p_value = p_value;
        ...
        if (pfn_simp_ble_service_cb)
        {
            pfn_simp_ble_service_cb(simp_service_id, (void *)&callback_data);
        }
    }
    else
    {
...
bool simp_ble_service_simple_v8_notify(uint8_t conn_id, T_SERVER_ID service_id, void *p_value,
                                       uint16_t length)
{
    APP_PRINT_INFO0("<-- simp_ble_service_simple_v8_notify");
    // send notification to client
    return server_send_data(conn_id, service_id, SIMPLE_BLE_SERVICE_CHAR_V8_NOTIFY_INDICATE_INDEX,
                            p_value, length, GATT_PDU_TYPE_NOTIFICATION);
}

bool simp_ble_service_simple_v8_indicate(uint8_t conn_id, T_SERVER_ID service_id, void *p_value,
                                         uint16_t length)
{
    APP_PRINT_INFO0("<-- simp_ble_service_simple_v8_indicate");
    // send indication to client
    return server_send_data(conn_id, service_id, SIMPLE_BLE_SERVICE_CHAR_V8_NOTIFY_INDICATE_INDEX,
                            p_value,
                                     ...
                                     uint16_t cccbits)
{
    ...
    case SIMPLE_BLE_SERVICE_CHAR_V8_CCCD_INDEX:
        {
            if (cccbits == GATT_CLIENT_CHAR_CONFIG_INDICATE)
            {
                // Enable Indication
                callback_data.msg_data.notification_indification_index = SIMP_NOTIFY_INDICATE_V8_INDICATE_ENABLE;
            }
            else if (cccbits == GATT_CLIENT_CHAR_CONFIG_NOTIFY)
            {
                // Disable Indication
                callback_data.msg_data.notification_indification_index = SIMP_NOTIFY_INDICATE_V8_NOTIFY_ENABLE;
            }
            else if (cccbits == GATT_CLIENT_CHAR_CONFIG_NOTIFY_INDICATE)
            {
                // Disable Indication
                callback_data.msg_data.notification_indification_index =
                    SIMP_NOTIFY_INDICATE_V8_NOTIFY_INDICATE_ENABLE;
            }
            else
            {
                // Disable Indication
                callback_data.msg_data.notification_indification_index = SIMP_NOTIFY_INDICATE_V8_DISABLE;
            }
