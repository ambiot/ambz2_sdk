...
#if F_BT_LE_GATT_CLIENT_SUPPORT
T_CLIENT_ID   SimBleClientID;
T_CLIENT_ID   GapClientID;
T_CLIENT_ID   vendor_tp_client_id;
T_CLIENT_ID   vendor_pxp_client_id;
#endif

uint8_t gSimpleProfileServiceId;
uint8_t g_vendor_pxpext_service_id;
uint8_t simple_srv_id;
uint8_t g_hid_keyboard_service_id;
...
#if F_BT_LE_GATT_SERVER_SUPPORT
    server_init(3);
    gSimpleProfileServiceId = vendor_tp_service_add((void *)app_profile_callback);
    g_vendor_pxpext_service_id = vendor_pxpext_service_add((void *)app_profile_callback);
...
#if F_BT_LE_GATT_CLIENT_SUPPORT
    client_init(3);
    /* Add Simple BLE Client Module, app_simp_client_callback used to handle events from this Module. */
    GapClientID = gaps_add_client(app_gap_client_callback, 4);
    vendor_tp_client_id = tp_client_add(test_client_callback);
    vendor_pxp_client_id = vendor_pxpext_client_add(test_client_callback);
    /* Register this callcack--app_general_client_callback to handle events from Profile Client layer. */
    client_register_general_client_cb(app_general_client_callback);
#endif
...
void driver_init()
{
    /* Use Data UART to handle user commands. */
