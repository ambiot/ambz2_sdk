...
*****************************************************************************************
  * @file    complete_ble_client.c
  * @brief   Complete Simple BLE client source file.
  * @details
  * @author  jane
  * @date    2016-02-18
  * @version v1.0
...
bool simp_ble_client_read_by_handle(uint8_t conn_id, T_SIMP_READ_TYPE read_type)
{
    ...
    case SIMP_READ_V7_READ_LONG:
        if (simp_table[conn_id].hdl_cache[HDL_SIMBLE_V7_READ_LONG])
        {
            handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_V7_READ_LONG];
            hdl_valid = true;
        }
        break;
    case SIMP_READ_V8_CCCD:
        if (simp_table[conn_id].hdl_cache[HDL_SIMBLE_V8_CCCD])
        {
...
bool simp_ble_client_read_by_uuid(uint8_t conn_id, T_SIMP_READ_TYPE read_type)
{
    ...
    case SIMP_READ_V7_READ_LONG:
        start_handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_SRV_START];
        end_handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_SRV_END];
        uuid16 = GATT_UUID_CHAR_SIMPLE_V7_READ_LONG;
        break;
    case SIMP_READ_V8_CCCD:
        start_handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_V8_CCCD];
        end_handle = simp_table[conn_id].hdl_cache[HDL_SIMBLE_SRV_END];
        uuid16 = GATT_UUID_CHAR_CLIENT_CONFIG;
        break;
    default:
...
bool simp_ble_client_set_v8_cccd(uint8_t conn_id, uint16_t cccd_bits)
{
    if (conn_id >= simp_link_num)
    {
        APP_PRINT_ERROR1("simp_ble_client_set_v8_cccd: failed invalid conn_id %d", conn_id);
        return false;
    }
    ...
    if (simp_table[conn_id].hdl_cache[HDL_SIMBLE_V8_CCCD])
    {
        ...
        uint16_t length = sizeof(uint16_t);
        uint8_t *p_data = (uint8_t *)&cccd_bits;
        if (client_attr_write(conn_id, simp_client, GATT_WRITE_TYPE_REQ, handle,
    ...
    APP_PRINT_WARN0("simp_ble_client_set_v8_cccd: Request fail! Please check!");
    return false;
}
...
bool simp_ble_client_write_v6_char(uint8_t conn_id, uint16_t length, uint8_t *p_value)
{
    if (conn_id >= simp_link_num)
    {
        APP_PRINT_ERROR1("simp_ble_client_write_v6_char: failed invalid conn_id %d", conn_id);
        return false;
    }

    if (simp_table[conn_id].hdl_cache[HDL_SIMBLE_V6_WRITE_LONG])
    {
    ...
    APP_PRINT_WARN0("simp_ble_client_write_v6_char: Request fail! Please check!");
    return false;
}
                                               ...
                                               T_DISCOVERY_RESULT_DATA result_data)
{
    ...
    if (simp_table[conn_id].disc_state == DISC_SIMP_START)
    {
        ...
        case DISC_RESULT_CHAR_UUID16:
            ...
            case GATT_UUID_CHAR_SIMPLE_V6_WRITE_LONG:
                hdl_cache[HDL_SIMBLE_V6_WRITE_LONG] = handle;
                break;

            case GATT_UUID_CHAR_SIMPLE_V7_READ_LONG:
                hdl_cache[HDL_SIMBLE_V7_READ_LONG] = handle;
                break;

            case GATT_UUID_CHAR_SIMPLE_V8:
                hdl_cache[HDL_SIMBLE_V8] = handle;
                break;
            ...
            if (result_data.p_char_desc_uuid16_disc_data->uuid16 == GATT_UUID_CHAR_CLIENT_CONFIG)
            {
                    ...
                    && (handle < hdl_cache[HDL_SIMBLE_V4_INDICATE]))
                {
                    hdl_cache[HDL_SIMBLE_V3_NOTIFY_CCCD] = handle;
                         ...
                         (handle < hdl_cache[HDL_SIMBLE_V6_WRITE_LONG]) &&
                         (hdl_cache[HDL_SIMBLE_V4_INDICATE_CCCD] == 0))
                {
                    hdl_cache[HDL_SIMBLE_V4_INDICATE_CCCD] = handle;
                }
                else if ((handle > hdl_cache[HDL_SIMBLE_V8])
                         && (hdl_cache[HDL_SIMBLE_V8_CCCD] == 0))
                {
                    hdl_cache[HDL_SIMBLE_V8_CCCD] = handle;
                }
            }
            break;
                                           ...
                                           uint16_t handle, uint16_t value_size, uint8_t *p_value)
{
    ...
    else if (handle == hdl_cache[HDL_SIMBLE_V7_READ_LONG])
    {
        cb_data.cb_content.read_result.type = SIMP_READ_V7_READ_LONG;
        if (cause == GAP_SUCCESS)
        {
            cb_data.cb_content.read_result.data.v7_read.p_value = p_value;
            cb_data.cb_content.read_result.data.v7_read.value_size = value_size;
        }
        else
        {
            cb_data.cb_content.read_result.data.v7_read.value_size = 0;
        }
    }
    else if (handle == hdl_cache[HDL_SIMBLE_V8_CCCD])
    {
        cb_data.cb_content.read_result.type = SIMP_READ_V8_CCCD;
        if (cause == GAP_SUCCESS)
        {
            uint16_t ccc_bit;
            if (value_size != 2)
            ...
            cb_data.cb_content.read_result.data.v8_notify_ind_cccd = ccc_bit;
        }
    }
    else
    {
                                            ...
                                            uint8_t credits)
{
    ...
    else if (handle == hdl_cache[HDL_SIMBLE_V6_WRITE_LONG])
    {
        cb_data.cb_content.write_result.type = SIMP_WRITE_V6_WRITE_LONG;
    }
    else if (handle == hdl_cache[HDL_SIMBLE_V8_CCCD])
    {
                                                        ...
                                                        uint16_t value_size, uint8_t *p_value)
{
    ...
    else if (handle == hdl_cache[HDL_SIMBLE_V8])
    {
        ...
        if (notify)
        {
            cb_data.cb_content.notif_ind_data.type = SIMP_V8_NOTIFY;

        }
        else
        {
            cb_data.cb_content.notif_ind_data.type = SIMP_V8_INDICATE;
        }
    }
    else
    {
