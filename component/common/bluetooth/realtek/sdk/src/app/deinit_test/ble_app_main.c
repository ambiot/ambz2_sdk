...
void app_init(void)
{
    APP_PRINT_INFO1("app_init s: %d", os_mem_peek(RAM_TYPE_DATA_ON));
    board_init();
    driver_init();
    ...
    pwr_mgr_init();
    app_task_init();
    APP_PRINT_INFO1("app_init end: %d", os_mem_peek(RAM_TYPE_DATA_ON));
}

bool upper_task_init(void)
{
    bool ret;
    APP_PRINT_INFO1("bte_init s: %d", os_mem_peek(RAM_TYPE_DATA_ON));
    bt_stack_config_init();
    ret = bte_init();
    APP_PRINT_INFO1("bte_init end: %d", os_mem_peek(RAM_TYPE_DATA_ON));
    return ret;
}

void app_deinit(void)
{
    APP_PRINT_INFO1("app_deinit s: %d", os_mem_peek(RAM_TYPE_DATA_ON));
    app_task_deinit();
    APP_PRINT_INFO1("app_deinit end: %d", os_mem_peek(RAM_TYPE_DATA_ON));
}

void upper_task_deinit(void)
{
    APP_PRINT_INFO1("upper_task_deinit s: %d", os_mem_peek(RAM_TYPE_DATA_ON));
    bte_deinit();
    APP_PRINT_INFO1("upper_task_deinit end: %d", os_mem_peek(RAM_TYPE_DATA_ON));
}

void user_cmd_task_init(void)
{
    uart_task_init();
}
#include "bt_board.h"
...
int ble_app_main(void)
{
    ...
    #if 0
    bt_stack_config_init();
    APP_PRINT_INFO1("next upper_task_init: %d", os_mem_peek(RAM_TYPE_DATA_ON));
    if (upper_task_init() == false)
    {
        APP_PRINT_ERROR0("ble_app_main: upper_task_init failed");
    }
    APP_PRINT_INFO1("next app_init: %d", os_mem_peek(RAM_TYPE_DATA_ON));
    app_init();
    APP_PRINT_INFO1("next user_cmd_task_init: %d", os_mem_peek(RAM_TYPE_DATA_ON));
    #endif
    user_cmd_task_init();
    APP_PRINT_INFO1("ble_app_main end: %d", os_mem_peek(RAM_TYPE_DATA_ON));
