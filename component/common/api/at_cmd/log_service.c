...
int parse_param(char *buf, char **argv)
{
	...
	int str_count = 0;
	int buf_cnt = 0;
	static char temp_buf[LOG_SERVICE_BUFLEN + 1];
...
void log_service(void *param)
{
	...
	while (1) {
#ifdef PLATFORM_OHOS
		while (rtw_down_sema(&log_rx_interrupt_sema) != _TRUE);
#else
...
#else
#ifdef PLATFORM_OHOS
	result = xTaskCreate(log_service, (const char *) "log_service", STACKSIZE, NULL, 6, &CreatedTask);
#else
