...
void fATSK(void *arg)
{
	...
	int ret;
	char *argv[MAX_ARGC] = {0};
	u8 key[PRIV_KEY_LEN],read_buffer[PRIV_KEY_LEN];
	flash_t flash;
	char *ptmp;
	...
	if(ret){
		at_printf("[ATSK] secure boot has been already enabled!\r\n");
		return;
	}
	...
	if(strcmp(argv[1], "ENC_KEY") == 0){
		...
		if(sscanf(argv[2], "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", 
			...
			device_mutex_unlock(RT_DEV_LOCK_EFUSE);
			if(ret < 0){
				dbg_printf("[ATSK] Err: efuse ss key: read address and length error\r\n");
				return;
			}
			for(i=0;i<PRIV_KEY_LEN;i++){
				if(read_buffer[i]!=0xFF)
				{
					dbg_printf("[ATSK] Err: the ENC Key has already been written.\r\n");
					return;
				}
			}
			//write SS key
			...
			if(memcmp(key,read_buffer,32)!=0){
				dbg_printf("[ATSK] Err: write ENC key failed.\r\n");
				return;
			}
			hal_delay_ms(10);
			}	
		...
		if(sscanf(argv[2], "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", 
			...
			device_mutex_unlock(RT_DEV_LOCK_EFUSE);
			if(ret < 0){
				dbg_printf("[ATSK] Err: efuse HASH key: read address and length error\r\n");
				return;
			}
			for(i=0;i<PRIV_KEY_LEN;i++){
				if(read_buffer[i]!=0xFF)
				{
					dbg_printf("[ATSK] Err: the HASH Key has been written already.\r\n");
					return;
				}
			}
			//write S key
			...
			device_mutex_unlock(RT_DEV_LOCK_EFUSE);
			if(memcmp(key,read_buffer,32)!=0){
				dbg_printf("[ATSK] Err: write HASH key failed.\r\n");
				return;
			}
			hal_delay_ms(10);
			}
