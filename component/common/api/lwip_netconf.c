...
#if LWIP_VERSION_MAJOR >= 2
					ip4_addr_set_u32(&dhcp->offered_ip_addr, (u32_t)offer_ip);
...
#if LWIP_VERSION_MAJOR >= 2
				dhcp = ((struct dhcp *)netif_get_client_data(pnetif, LWIP_NETIF_CLIENT_DATA_INDEX_DHCP));
				restore_wifi_info_to_flash((uint32_t)ip4_addr_get_u32(&dhcp->offered_ip_addr), (uint32_t)ip4_addr_get_u32(ip_2_ip4(&dhcp->server_ip_addr)));
#else
				dhcp = pnetif->dhcp;
